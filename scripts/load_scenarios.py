...
Usage:
    ...
    OPENOMA_SERVER_URL=http://localhost:8000 uv run python scripts/load_scenarios.py
"""
...
SERVER = os.environ.get("OPENOMA_SERVER_URL", "http://localhost:8000")
...
def gql(query: str, variables: dict | None = None) -> dict:
    ...
    resp = httpx.post(GQL, json={"query": query, "variables": variables or {}}, timeout=15)
    resp.raise_for_status()
    body = resp.json()
    if body.get("errors"):
        print(f"\n❌ GraphQL errors:\n{json.dumps(body['errors'], indent=2)}", file=sys.stderr)
...
def scenario_work_blocks() -> dict[str, str]:
    ...
    definitions = [
        {
            ...
            "inputs": [
                {"name": "ticket", "description": "Raw ticket payload", "required": True,
                 "schema": {"type": "object", "properties": {"title": {"type": "string"}, "body": {"type": "string"}}}},
            ],
            "outputs": [
                {"name": "severity", "description": "P0–P4 classification", "required": True,
                 "schema": {"type": "string", "enum": ["P0", "P1", "P2", "P3", "P4"]}},
                {"name": "assigned_team", "description": "Team handle", "required": True},
            ],
            "executionHints": ["ticket-system-access", "triage-trained"],
            "expectedOutcome": {"name": "Triage done", "description": "Every ticket classified within SLA"},
        ...
        },
        {
            ...
            "inputs": [
                ...
                {"name": "codebase_ref", "description": "Git ref to branch from", "required": False},
            ],
            "outputs": [
                {"name": "pull_request_url", "description": "URL of the PR", "required": True},
                {"name": "test_results", "description": "CI output", "required": True,
                 "schema": {"type": "object", "properties": {"passed": {"type": "integer"}, "failed": {"type": "integer"}}}},
            ],
            "executionHints": ["codebase-write", "ci-access"],
            "expectedOutcome": {"name": "Code complete", "description": "PR passing CI with no critical issues"},
        ...
        },
        {
            ...
            "inputs": [
                {"name": "pull_request_url", "description": "URL of the PR to review", "required": True},
                {"name": "review_checklist", "description": "Criteria for approval", "required": False},
            ],
            "outputs": [
                {"name": "verdict", "description": "approved | changes_requested | rejected", "required": True,
                 "schema": {"type": "string", "enum": ["approved", "changes_requested", "rejected"]}},
                {"name": "comments", "description": "Inline review comments", "required": True},
            ],
        ...
        },
        {
            ...
            "inputs": [
                {"name": "environment", "description": "staging | production", "required": True},
            ],
            "outputs": [
                {"name": "is_live", "description": "Boolean health status", "required": True,
                 "schema": {"type": "boolean"}},
                {"name": "deploy_sha", "description": "Git SHA currently deployed", "required": True},
            ],
        ...
        },
        {
            ...
            "outputs": [
                {"name": "results", "description": "Test results summary", "required": True,
                 "schema": {"type": "object"}},
            ],
        ...
        },
        {
            ...
            "inputs": [
                {"name": "is_live", "description": "Deployment health", "required": True},
                {"name": "smoke_results", "description": "Smoke test output", "required": True},
            ],
            "outputs": [
                {"name": "verdict", "description": "pass | fail", "required": True,
        ...
        },
        {
            ...
            "inputs": [
                {"name": "recipients", "description": "List of emails", "required": True,
        ...
        },
        {
            ...
            "inputs": [
                {"name": "survey_ids", "description": "Tracking IDs to watch", "required": True},
            ],
            "outputs": [
                {"name": "responses", "description": "Aggregated response data", "required": True,
                 "schema": {"type": "array"}},
            ],
        ...
        },
        {
            ...
            "inputs": [
                {"name": "responses", "description": "Raw survey responses", "required": True},
            ],
            "outputs": [
                {"name": "csat_score", "description": "Score from 1.0 to 5.0", "required": True,
                 "schema": {"type": "number", "minimum": 1.0, "maximum": 5.0}},
                {"name": "breakdown", "description": "Per-question scores", "required": False},
            ],
            "executionHints": [],
            "metadata": {"category": "assessment"},
        },
        {
            ...
            "inputs": [
                {"name": "targets", "description": "List of endpoints to check", "required": True},
            ],
            "outputs": [
                {"name": "status", "description": "Overall status", "required": True,
                 "schema": {"type": "string", "enum": ["healthy", "degraded", "down"]}},
        ...
        },
        {
            ...
            "inputs": [
                {"name": "issue_type", "description": "Type of issue detected", "required": True},
                {"name": "target", "description": "Affected resource", "required": True},
            ],
            "outputs": [
                {"name": "repair_status", "description": "success | failed | escalated", "required": True},
            ],
        ...
        },
        {
            ...
            "inputs": [
                {"name": "metrics", "description": "Time-series health data", "required": True},
                {"name": "window", "description": "Evaluation window in hours", "required": True},
            ],
            "outputs": [
                {"name": "uptime_pct", "description": "Percentage uptime", "required": True,
                 "schema": {"type": "number", "minimum": 0, "maximum": 100}},
                {"name": "incidents", "description": "Downtime incidents list", "required": False},
            ],
            "executionHints": [],
            "metadata": {"category": "assessment"},
        },
    ...
    for defn in definitions:
        ...
        check(
            ...
            wb is not None and wb.get("name") == defn["name"] and wb.get("version") == 1,
        )
        if wb:
            blocks[wb["name"]] = wb["id"]
            if defn.get("executionHints"):
                check(f"  execution_hints round-trip", wb["executionHints"] == defn["executionHints"])
            if defn.get("expectedOutcome"):
                check(f"  expected_outcome round-trip", wb["expectedOutcome"]["name"] == defn["expectedOutcome"]["name"])
...
def scenario_flows(blocks: dict[str, str]) -> dict[str, str]:
    ...
    sprint_input = {
        ...
        "nodes": [
            {"id": NODE_TRIAGE, "targetId": blocks["Ticket Triage"], "targetVersion": 1,
             "alias": "triage", "metadata": {"position": {"x": 100, "y": 300}}},
            {"id": NODE_IMPL_A, "targetId": blocks["Implementation"], "targetVersion": 1,
             "alias": "normal-impl", "metadata": {"position": {"x": 400, "y": 150}}},
            {"id": NODE_IMPL_B, "targetId": blocks["Implementation"], "targetVersion": 1,
             "alias": "fast-track-impl", "metadata": {"position": {"x": 400, "y": 450}}},
            {"id": NODE_REVIEW, "targetId": blocks["Code Review"], "targetVersion": 1,
             "alias": "review", "metadata": {"position": {"x": 700, "y": 300}}},
        ],
        "edges": [
            # Entry edge: triage is a starting node
            {"targetId": NODE_TRIAGE},
            # Conditional: approved ticket → normal implementation
            {"sourceId": NODE_TRIAGE, "targetId": NODE_IMPL_A,
             "condition": {"description": "ticket is approved for standard process",
            ...
            {"sourceId": NODE_TRIAGE, "targetId": NODE_IMPL_B,
             "condition": {"description": "ticket is fast-tracked",
                           "predicate": {"op": "in", "field": "severity", "value": ["P0", "P1"]}}},
            # Both implementations converge to code review (unconditional)
            {"sourceId": NODE_IMPL_A, "targetId": NODE_REVIEW,
             "portMappings": [{"sourcePort": "pull_request_url", "targetPort": "pull_request_url"}]},
            {"sourceId": NODE_IMPL_B, "targetId": NODE_REVIEW,
             "portMappings": [{"sourcePort": "pull_request_url", "targetPort": "pull_request_url"}]},
        ],
        "expectedOutcome": {"name": "Sprint delivered", "description": "All sprint tickets implemented and reviewed"},
    ...
    check("Flow 'Engineering Sprint Cycle' created", flow is not None and flow["version"] == 1)
    if flow:
        flows[flow["name"]] = flow["id"]
        check("  4 nodes (same-block-two-slots pattern)", len(flow["nodes"]) == 4)
        check("  5 edges (1 entry + 2 conditional + 2 unconditional)", len(flow["edges"]) == 5)
    ...
    release_input = {
        ...
        "nodes": [
            {"id": NODE_CHECK_DEPLOY, "targetId": blocks["Check Deployment Status"], "targetVersion": 1,
             "alias": "check-deploy"},
            {"id": NODE_RUN_SMOKE, "targetId": blocks["Run Smoke Tests"], "targetVersion": 1,
             "alias": "smoke"},
            {"id": NODE_VERDICT, "targetId": blocks["Produce Release Verdict"], "targetVersion": 1,
             "alias": "verdict"},
        ],
        "edges": [
            {"targetId": NODE_CHECK_DEPLOY},
            {"sourceId": NODE_CHECK_DEPLOY, "targetId": NODE_RUN_SMOKE,
             "portMappings": [{"sourcePort": "deploy_sha", "targetPort": "environment"}]},
            {"sourceId": NODE_RUN_SMOKE, "targetId": NODE_VERDICT,
             "portMappings": [{"sourcePort": "results", "targetPort": "smoke_results"}]},
        ],
        "expectedOutcome": {"name": "Release verdict", "description": "Binary pass/fail on release readiness"},
    }
    body = gql("""
    mutation CreateFlow($input: CreateFlowInput!) {
    ...
    if flow:
        ...
        terminal = [n for n in flow["nodes"]
    ...
    csat_input = {
        ...
        "nodes": [
            {"id": NODE_COLLECT_SURVEY, "targetId": blocks["Collect Survey Responses"], "targetVersion": 1,
             "alias": "collect"},
            {"id": NODE_COMPUTE_CSAT, "targetId": blocks["Compute CSAT Score"], "targetVersion": 1,
             "alias": "compute"},
        ],
        "edges": [
            {"targetId": NODE_COLLECT_SURVEY},
            {"sourceId": NODE_COLLECT_SURVEY, "targetId": NODE_COMPUTE_CSAT,
             "portMappings": [{"sourcePort": "responses", "targetPort": "responses"}]},
        ],
    }
    body = gql("""
    mutation CreateFlow($input: CreateFlowInput!) {
    ...
    smoke_input = {
        ...
        "nodes": [
            {"id": NODE_SMOKE_ENTRY, "targetId": blocks["Check Deployment Status"], "targetVersion": 1,
             "alias": "entry-check"},
            {"id": NODE_SMOKE_RUN, "targetId": blocks["Run Smoke Tests"], "targetVersion": 1,
             "alias": "run"},
            {"id": NODE_SMOKE_REPORT, "targetId": blocks["Produce Release Verdict"], "targetVersion": 1,
             "alias": "report"},
        ],
        "edges": [
    ...
    survey_input = {
        ...
        "nodes": [
            {"id": NODE_SURVEY_SEND, "targetId": blocks["Send Survey"], "targetVersion": 1, "alias": "send"},
            {"id": NODE_SURVEY_COLLECT, "targetId": blocks["Collect Survey Responses"], "targetVersion": 1, "alias": "collect"},
        ],
        "edges": [
            {"targetId": NODE_SURVEY_SEND},
            {"sourceId": NODE_SURVEY_SEND, "targetId": NODE_SURVEY_COLLECT,
             "portMappings": [{"sourcePort": "survey_ids", "targetPort": "survey_ids"}]},
        ],
    }
    body = gql("""
    mutation CreateFlow($input: CreateFlowInput!) {
    ...
    infra_input = {
        ...
        "nodes": [
            {"id": NODE_MONITOR_A, "targetId": blocks["Infrastructure Health Check"], "targetVersion": 1,
             "alias": "monitor-primary"},
            {"id": NODE_MONITOR_B, "targetId": blocks["Infrastructure Health Check"], "targetVersion": 1,
             "alias": "monitor-secondary"},
            {"id": NODE_INFRA_CHECK, "targetId": blocks["Infrastructure Health Check"], "targetVersion": 1,
             "alias": "deep-check"},
            {"id": NODE_INFRA_REPAIR, "targetId": blocks["Automated Repair"], "targetVersion": 1,
             "alias": "repair"},
            {"id": NODE_INFRA_VERIFY, "targetId": blocks["Infrastructure Health Check"], "targetVersion": 1,
        ...
        "edges": [
            ...
            {"sourceId": NODE_INFRA_CHECK, "targetId": NODE_INFRA_REPAIR,
             "condition": {"description": "status is degraded or down",
                           "predicate": {"op": "in", "field": "status", "value": ["degraded", "down"]}}},
    ...
    if flow:
        flows[flow["name"]] = flow["id"]
        entry_edges = [e for e in flow["edges"] if e["sourceId"] is None]
        check("  3 entry edges (3 starting points in 2 subgraphs)", len(entry_edges) == 3)
        check("  5 nodes (same block used 4 times = different slots)", len(flow["nodes"]) == 5)
    ...
    uptime_input = {
        ...
        "nodes": [
            {"id": NODE_UPTIME_EVAL, "targetId": blocks["Uptime Evaluation"], "targetVersion": 1,
             "alias": "eval"},
        ],
        "edges": [
    ...
    """, {"input": uptime_input})
    flow = extract(body, "data.createFlow")
    check("Flow 'Uptime Monitor Assessment' created (single-node assessment)", flow is not None)
    if flow:
        flows[flow["name"]] = flow["id"]
...
def scenario_required_outcomes(flows: dict[str, str]) -> dict[str, str]:
    ...
    """, {"input": {
        ...
        "assessmentBindings": [{
            "assessmentFlow": {"flowId": flows["Release Verification"], "flowVersion": 1},
            "testFlowRefs": [
                {"flowId": flows["Smoke Test Suite"], "flowVersion": 1, "alias": "smoke"},
            ],
        }],
        "metadata": {"priority": "critical"},
    }})
    ro = extract(body, "data.createRequiredOutcome")
    check("RequiredOutcome 'Feature X shipped' created", ro is not None and ro["version"] == 1)
    if ro:
        outcomes[ro["name"]] = ro["id"]
        check("  has 1 assessment binding", len(ro["assessmentBindings"]) == 1)
        check("  binding has 1 test flow ref", len(ro["assessmentBindings"][0]["testFlowRefs"]) == 1)
    ...
    """, {"input": {
        ...
        "assessmentBindings": [{
            "assessmentFlow": {"flowId": flows["CSAT Evaluator"], "flowVersion": 1},
            "testFlowRefs": [
                {"flowId": flows["Survey Collection"], "flowVersion": 1, "alias": "survey-data"},
            ],
        }],
    }})
    ro = extract(body, "data.createRequiredOutcome")
    ...
    """, {"input": {
        ...
        "assessmentBindings": [{
            "assessmentFlow": {"flowId": flows["Uptime Monitor Assessment"], "flowVersion": 1},
        }],
    }})
    ro = extract(body, "data.createRequiredOutcome")
...
def scenario_contracts(flows: dict[str, str], outcomes: dict[str, str]) -> dict[str, str]:
    ...
    """, {"input": {
        ...
        "owners": [
            {"name": "Alice Chen", "role": "lead", "contact": "alice@example.com"},
        ...
        "requiredOutcomes": [
            {"requiredOutcomeId": outcomes["99.9% uptime"], "requiredOutcomeVersion": 1},
        ],
    ...
    if c:
        ...
        check("  lead is Alice Chen", c["owners"][0]["name"] == "Alice Chen" and c["owners"][0]["role"] == "lead")
    ...
    """, {"input": {
        ...
        "owners": [
            {"name": "Bob Martinez", "role": "lead", "contact": "bob@example.com"},
            {"name": "Carol Wang", "role": "approver", "contact": "carol@example.com"},
            {"name": "Dev Team", "role": "delegate"},
            {"name": "QA Lead", "role": "reviewer", "contact": "qa-lead@example.com"},
        ],
        "workFlows": [
            {"flowId": flows["Engineering Sprint Cycle"], "flowVersion": 1, "alias": "eng-sprints"},
        ],
        "subContracts": [
            {"contractId": contracts["Infrastructure SLA"], "contractVersion": 1, "alias": "infra-sla"},
        ],
        "requiredOutcomes": [
            {"requiredOutcomeId": outcomes["Feature X shipped to production"], "requiredOutcomeVersion": 1, "alias": "feature-shipped"},
            {"requiredOutcomeId": outcomes["Customer satisfaction ≥ 4.5/5"], "requiredOutcomeVersion": 1, "alias": "csat"},
        ],
    ...
    if c:
        contracts[c["name"]] = c["id"]
        check("  has 4 owners (lead + approver + delegate + reviewer)", len(c["owners"]) == 4)
        check("  has 1 work flow", len(c["workFlows"]) == 1)
        check("  has 1 sub-contract", len(c["subContracts"]) == 1)
        check("  has 2 required outcomes", len(c["requiredOutcomes"]) == 2)
        check("  sub-contract alias is 'infra-sla'", c["subContracts"][0]["alias"] == "infra-sla")
...
def scenario_versioning(blocks: dict[str, str]):
    ...
    """, {"id": wb_id, "input": {
        ...
        "metadata": {"category": "intake", "sla_minutes": 10, "ai_assisted": True},
    }})
    ...
    check("Version 1 has original description", "AI-assisted" not in (v1 or {}).get("description", ""))
    ...
    check("Version 2 has updated description", "AI-assisted" in (v2 or {}).get("description", ""))
...
def scenario_versioning_flow(flows: dict[str, str]):
    ...
    """, {"id": flow_id, "input": {
        "metadata": {"sprint_length_days": 14, "canvas_zoom": 1.2, "updated": True},
    }})
    updated = extract(body, "data.updateFlow")
    check("Flow versioned to v2", updated is not None and updated["version"] == 2)

    # Both versions accessible
    body = gql("query($id: UUID!) { flow(id: $id, version: 1) { version } }", {"id": flow_id})
...
def scenario_execution_lifecycle(blocks: dict[str, str], flows: dict[str, str], contracts: dict[str, str]):
    ...
    check("ContractExecution created (pending)", ce is not None and ce["state"] == "pending")
    ce_id = ce["id"]
    ...
    """, {"input": {"flowId": sprint_flow_id, "flowVersion": 1, "contractExecutionId": ce_id}})
    fe = extract(body, "data.createFlowExecution")
    check("FlowExecution created (pending)", fe is not None and fe["state"] == "pending")
    fe_id = fe["id"]
    ...
    mutation($ceId: UUID!, $feId: UUID!) {
      addFlowToContractExecution(contractExecutionId: $ceId, flowExecutionId: $feId) {
        id flowExecutions state
      }
    }
    """, {"ceId": ce_id, "feId": fe_id})
    ce_updated = extract(body, "data.addFlowToContractExecution")
    check("Flow linked to contract execution", fe_id in (ce_updated or {}).get("flowExecutions", []))
    ...
    check("BlockExecution (triage) created", be_triage is not None and be_triage["state"] == "pending")
    ...
    """, {"id": be_triage_id, "exec": {"type": "human", "identifier": "alice@example.com",
                                        "metadata": {"team": "triage"}}})
    be = extract(body, "data.assignExecutorToBlock")
    check("Executor assigned to triage block", be is not None and be["state"] == "pending")
    ...
    check("Triage block started (in_progress)", be is not None and be["state"] == "in_progress")
    ...
    check("  created → executor_assigned → started → outcome_produced → completed",
          event_types == ["created", "executor_assigned", "started", "outcome_produced", "completed"])

    # Check that the executor info round-tripped on the assigned event
    assigned_evt = next((e for e in (events or []) if e["eventType"] == "executor_assigned"), None)
    check("  executor_assigned carries ExecutorInfo",
          assigned_evt is not None and assigned_evt["executor"]["identifier"] == "alice@example.com")
...
def scenario_executor_handoff(blocks: dict[str, str]):
    ...
    """, {"id": be_id, "exec": {"type": "agent", "identifier": "agent-A", "metadata": {"model": "gpt-4"}}})

    # Agent A starts
    gql("mutation($id: UUID!) { startBlockExecution(executionId: $id) { id } }", {"id": be_id})
    ...
    """, {"id": be_id, "exec": {"type": "agent", "identifier": "agent-B", "metadata": {"model": "claude-4"}}})
    ...
    """, {"id": be_id, "outcome": {"value": {"pr_url": "https://github.com/example/pr/42"}}})
    gql("mutation($id: UUID!) { completeBlockExecution(executionId: $id) { id } }", {"id": be_id})
    ...
    check("Handoff event stream recorded",
    ...
    check("  agent-A and agent-B both appear in log",
...
def scenario_flow_state_derivation(blocks: dict[str, str], flows: dict[str, str]):
    ...
    block_ids = []
    for node_id, wb_name in [(NODE_CHECK_DEPLOY, "Check Deployment Status"),
        ...
        """, {"input": {
            "workBlockId": blocks[wb_name], "workBlockVersion": 1,
            "nodeReferenceId": node_id, "flowExecutionId": fe_id,
        }})
    ...
    check("All blocks pending → flow pending", extract(body, "data.flowExecution.state") == "pending")
    ...
    gql(f'mutation {{ refreshFlowState(flowExecutionId: "{fe_id}") {{ id state }} }}')
    body = gql(f'{{ flowExecution(id: "{fe_id}") {{ state }} }}')
    check("One block started → flow in_progress", extract(body, "data.flowExecution.state") == "in_progress")
    ...
    gql(f'mutation {{ refreshFlowState(flowExecutionId: "{fe_id}") {{ id state }} }}')
    body = gql(f'{{ flowExecution(id: "{fe_id}") {{ state }} }}')
    check("All blocks completed → flow completed", extract(body, "data.flowExecution.state") == "completed")
    ...
    """, {"input": {
        "workBlockId": blocks["Check Deployment Status"], "workBlockVersion": 1,
        "nodeReferenceId": NODE_CHECK_DEPLOY, "flowExecutionId": fe2_id,
    }})
    fail_be_id = extract(body, "data.createBlockExecution.id")
    gql(f'mutation {{ addBlockToFlowExecution(flowExecutionId: "{fe2_id}", blockExecutionId: "{fail_be_id}") {{ id }} }}')
    ...
    gql(f'mutation {{ failBlockExecution(executionId: "{fail_be_id}") {{ id state }} }}')

    gql(f'mutation {{ refreshFlowState(flowExecutionId: "{fe2_id}") {{ id }} }}')
    body = gql(f'{{ flowExecution(id: "{fe2_id}") {{ state }} }}')
    check("Any block failed → flow failed", extract(body, "data.flowExecution.state") == "failed")
...
def scenario_many_to_many(blocks: dict[str, str], flows: dict[str, str]):
    ...
    """, {"input": {
        "workBlockId": blocks["Run Smoke Tests"], "workBlockVersion": 1,
    ...
    gql(f'mutation {{ completeBlockExecution(executionId: "{shared_be_id}") {{ id }} }}')
    ...
    for _ in range(2):
        ...
        gql(f'mutation {{ addBlockToFlowExecution(flowExecutionId: "{fid}", blockExecutionId: "{shared_be_id}") {{ id blockExecutions }} }}')
        fe_ids.append(fid)

    # Verify both flow executions reference the same block execution
    for i, fid in enumerate(fe_ids):
        body = gql(f'{{ flowExecution(id: "{fid}") {{ blockExecutions }} }}')
        bes = extract(body, "data.flowExecution.blockExecutions")
        check(f"FlowExecution {i+1} references shared block", shared_be_id in (bes or []))
...
def scenario_queries(blocks: dict[str, str]):
    ...
    wbs = extract(body, "data.workBlocks")
    check("Filter workBlocks by name", wbs is not None and len(wbs) >= 1 and wbs[0]["name"] == "Code Review")
    ...
    check("Pagination works (page1 ≠ page2)", page1 is not None and page2 is not None
          and len(page1) == 3 and set(p["name"] for p in page1) != set(p["name"] for p in page2))
    ...
    check(f"List requiredOutcomes returns ≥3 results", ros is not None and len(ros) >= 3)
    ...
    check("Filter blockExecutions by state=completed",
...
def main():
    ...
    if fail_count > 0:
        ...
        print("  ✅ Identity and immutability (versioned entities, both versions queryable)")
